      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          <table className="w-full">
            ...
            <tbody>{appointments.map(a => (<tr key={a.id} className={a.status === 'checked_in' ? 'bg-green-50' : ''}><td className="px-6 py-4 font-bold text-2xl">#{a.queue_number}</td><td className="px-6 py-4">{a.service_name}</td><td className="px-6 py-4">{a.start_time}</td><td className="px-6 py-4"><span className="inline-block px-3 py-1 rounded-full text-sm font-medium bg-yellow-100 text-yellow-700">{a.status}</span></td><td className="px-6 py-4">{a.verified_by_fingerprint ? <CheckCircle className="h-5 w-5 text-green-600"/> : <X className="h-5 w-5 text-gray-300"/>}</td></tr>))}{appointments.length === 0 && <tr><td colSpan={5} className="px-6 py-16 text-center text-gray-500"><Users className="h-12 w-12 mx-auto mb-3 text-gray-300"/><p className="text-lg">No appointments today.</p></td></tr>}</tbody>
          </table>
        </div>
      </div>
    </div>
