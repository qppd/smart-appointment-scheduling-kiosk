...
import { Mail, Phone, Calendar, MapPin, Lock, AlertCircle } from 'lucide-react';
...
export default function Register() {
  ...
  const [form, setForm] = useState({
    first_name: '', last_name: '', middle_name: '', email: '', phone: '', birth_date: '', address: '', password: '', confirm_password: ''
  });
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    if (!form.phone.trim()) { setError('Phone number is required for OTP verification'); return; }
    setLoading(true);
    try {
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    e.preventDefault();
    setOtpError('');
    if (!otp.trim()) { setOtpError('Please enter the 6-digit OTP'); return; }
    setOtpLoading(true);
    try {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        ...
        {/* OTP Step - shown when user has been sent an OTP */}
        {step === 'otp' && (
          <div className="space-y-5">
            <h2 className="text-xl font-bold text-gray-900 mb-4 text-center">Enter OTP</h2>
            <p className="text-sm text-gray-500 text-center">We've sent a 6-digit code to {form.phone}</p>
            <form onSubmit={handleVerifyOTP} className="space-y-4">
              {otpError && <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg flex items-center gap-2"><AlertCircle className="h-5 w-5" /><span>{otpError}</span></div>}
              <div className="flex justify-center">
                <input type="text" maxLength={6} value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, ''))} className="w-48 text-center text-2xl tracking-[0.5em] px-4 py-3 border border-gray-300 rounded-lg" placeholder="______" required />
              </div>
              <button type="submit" disabled={otpLoading || otp.length !== 6} className="w-full bg-teal-600 text-white py-3 rounded-lg font-semibold hover:bg-teal-700 disabled:opacity-50">{otpLoading ? 'Verifying...' : 'Verify & Register'}</button>
            </form>
            <button onClick={handleResend} disabled={countdown > 0} className="w-full text-center text-sm text-teal-600 hover:text-teal-700 disabled:text-gray-400">{countdown > 0 ? `Resend in ${countdown}s` : 'Resend OTP'}</button>
            <button onClick={() => setStep('form')} className="w-full text-center text-sm text-gray-500 hover:text-gray-700">Back to Registration</button>
          </div>
        )}

        {/* Form Step - shown initially */}
        {step === 'form' && (
          <>
            <h1 className="text-2xl font-bold text-gray-900 mb-6 text-center">Register Your Account</h1>
            {error && <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg mb-6 flex items-start gap-2"><AlertCircle className="h-5 w-5 mt-0.5" /><span>{error}</span></div>}
            <form onSubmit={handleSendOTP} className="space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><div><label className="block text-sm font-medium text-gray-700 mb-1">Address *</label><div className="relative"><MapPin className="absolute left-3 top-2.5 h-5 w-5 text-gray-400" /><input type="text" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} className="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg" required /></div></div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <button type="submit" disabled={loading} className="w-full bg-teal-600 text-white py-3 rounded-lg font-semibold hover:bg-teal-700 disabled:opacity-50">{loading ? 'Sending OTP...' : 'Register & Send OTP'}</button>
            </form>
