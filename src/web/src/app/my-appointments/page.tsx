...
export default function MyAppointments() {
  ...
  useEffect(() => {
    ...
    onAuthChange((user) => {
      ...
      unsub = subscribeMyAppointments(user.uid, (apts) => { setAppointments(apts); setLoading(false); });
    });
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {!loading && appointments.length === 0 && (
        <div className="text-center py-16"><Calendar className="h-12 w-12 text-gray-300 mx-auto mb-4" /><p className="text-gray-500">No appointments found.</p></div>
      )}
      <div className="space-y-4">{appointments.map(apt => (
